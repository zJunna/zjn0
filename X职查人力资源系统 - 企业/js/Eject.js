function Eject() {
	...
	_this.Forceconfirm = function(obj) {
		var confirmBox = $('<div class="alertBox"></div>')
		var confirmMes = $('<div class="alertMes">' + obj.message + '</div>')
		var confirmBtn = $('<span class="ConBtn">取消签约</span>').on('click', function() {
			qback.remove()
			confirmBox.remove()
			setTimeout(function() {
				obj.define()
		...
		var confirmcancel = $('<span class="cancel">继续签约</span>').on('click', function() {
			qback.remove()
			confirmBox.remove()
			setTimeout(function() {
				obj.cancel()
